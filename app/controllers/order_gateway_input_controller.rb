  ...
  def create
    ...
    respond_to do |format|
      ...
      if @orders.size > 0 && @errors.size < 1 
        ...
        SogiExtension.on_importing_error("OrderCreator Exceptions (#{params[:origin_account_short_name]} #{params[:origin_account_transaction_identifier]}) : #{@errors.first ? @errors.first.message : ''}", error_message) # if SogiExtension.respond_to?(:on_importing_error) 
