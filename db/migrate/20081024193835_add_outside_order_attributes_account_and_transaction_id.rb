class AddOutsideOrderAttributesAccountAndTransactionId < ActiveRecord::Migration
  def self.up
    ...
    add_column(:outside_order_attributes, :origin_account_transaction_id, :string) # so we know how to confirm this order
  end

  def self.down
    remove_column(:outside_order_attributes, :origin_account_transaction_id) # so we know how to confirm this order
